...
#Function to calculate crowding distance
    """
    Crowding distance is calculated by first sorting the set of solutions
in ascending objective function values. The crowding distance
value of a particular solution is the average distance of its two
neighboring solutions. The boundary solutions which have the
lowest and highest objective function values are given an infinite
crowding distance values so that they are always selected. This
process is done for each objective function. The final crowding
distance value of a solution is computed by adding the entire
individual crowding distance values in each objective function. 
    """
    def crowding_distance(self,values1, values2, front, sde = False):
        ...
        for i in range(0, len(front)):
            ...
            try:
                ...
                distance[i] = (values1[sorted1[sorted1_index+1]] - values1[sorted1[sorted1_index-1]])/(max(values1)-min(values1))
                #print("Distance value 1: ", distance[i])
            except Exception:
                pass
            try:
                sorted2_index = sorted2.index(k)
                distance[i] = (values2[sorted2[sorted2_index+1]] - values2[sorted2[sorted2_index-1]])/(max(values2)-min(values2))
                #print("Distance value 2: ", distance[i])
            except Exception:
                pass

        min_distance = min(distance)
        min_distance_index = distance.index(min_distance)
        distance.remove(min_distance)
