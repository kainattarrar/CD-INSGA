...
from algorithm.Transfer_Initialization import Transfer_Initializer
...
class TMOGA:
    def __init__(self,graph_list,pop_size = 200, max_gen = 100, CID = 0.8, transfer_prob = 0.5, mutation_prob = 0.2, crossover_prob = 0.8, max_num_cliques = 5, first_round_generation = None, sde = False):
    ...
    # Community Score measures the quality of the community structure results.
        ...
        for i in range(pop_size):
            ...
            for node in list(graph.nodes()):
                if len(list(graph.neighbors(node))) == 0:
                    ...
                    locus = choice(list(graph.neighbors(node))) #A locus is the set of all points (usually forming a curve or surface) satisfying some condition
    ...
    def start(self):
        ...
        for i in tqdm(range(len(self.graph_list))):
            if i == 0:
                ...
                solution = NG2.start() #running NSGA algorithm on population
                solution_values =  [self.__evaluate_solution(self.graph_list[i],solution[k]) for k in range(len(solution))]  #finding community score
                ...
                solution_values = [self.__evaluate_solution(self.graph_list[i],solutions[k]) for k in range(len(solutions))]  #calculation of community score
                best_solution = solutions[solution_values.index(max(solution_values))] #Solution with highest community score
                part_solution.append(best_solution)
    ...
    def PGLP(self, graph, iters = 5): #Label Propagation used to generate 1st solution
    ...
    def __perturbation(self, x):
        ...
        i = n - 1
        while  i >= 0:
            index = randint(0, n - 1)  #The randint() method returns an integer number selected element from the specified range.
